...
> 版本: v0.3.0 | 架构: 集中式监控平台 + 轻量 Agent + 按节点 Token 一键部署
...
管理员（看板）                               VyOS 节点
 ...
 显示 curl|bash 命令 [📋复制]                   │
    ...
    │                                          │
    │                                          ▼
    │                            3. curl -sL .../api/deploy/tk_xxx | bash
    │                               ├── 创建 /opt/vyomni-agent/
    │                               ├── 下载 agent_common.py + collector.py
    │                               ├── 写入 config.conf（含 token）
    │                               ├── 创建 systemd 服务
    │                               └── 启动 → 自动注册
    │                                          │
    │                                POST /register (携带 tk_xxx)
    │                                          │
    ▼                                          ▼
 4. 平台验证 Token                    Agent 获得专属 hmac_key
    ├── Token 作废（一次性）            └── 开始定期上报
    └── 节点状态: pending
    │
    ▼
 5. 管理员审核 → approved → 看板展示数据
```
...
| 使用次数 | 一次性（注册成功即作废） |
| 泄露影响 | 仅单节点 + 时效限制 |
| 注册后通信 | 使用平台分配的专属 HMAC 密钥（与 Token 无关） |

### API 端点

| 端点 | 方法 | 说明 |
|------|------|------|
| `/api/tokens/generate` | POST | 生成 Token（需 admin 权限） |
| `/api/tokens` | GET | 列出所有 Token（unused/used/expired） |
| `/api/deploy/{token}` | GET | 返回 bash 部署脚本 |
